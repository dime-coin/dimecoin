...
/** C++ wrapper with unsigned char Span interface around poly1305_donna code. */
class Poly1305
{
...
public:
    ...
    Poly1305(Span<const unsigned char> key) noexcept
    {
        assert(key.size() == KEYLEN);
        poly1305_donna::poly1305_init(&m_ctx, key.data());
    }

    /** Process message bytes. */
    Poly1305& Update(Span<const unsigned char> msg) noexcept
    {
        poly1305_donna::poly1305_update(&m_ctx, msg.data(), msg.size());
        return *this;
    }

    /** Write authentication tag to 16-byte out. */
    void Finalize(Span<unsigned char> out) noexcept
    {
        assert(out.size() == TAGLEN);
        poly1305_donna::poly1305_finish(&m_ctx, out.data());
    }
};
