...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " dimecoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  dimecoin-tx [options] <hex-tx> [commands]  Update hex-encoded dimecoin transaction\n" +
            "or:     dimecoin-tx [options] -create [commands]   Create hex-encoded dimecoin transaction\n" +
            "\n";
...
static CAmount AmountFromValue(const UniValue& value)
{
    ...
    CAmount amount;
    if (!ParseFixedPoint(value.getValStr(), 5, &amount))
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dimecoin transaction
