...
bool CPrivateSend::IsCollateralValid(const CTransactionRef& txCollateral)
{
...

    {
        ...
        if(!AcceptToMemoryPool(mempool, validationState, txCollateral, nullptr, nullptr, false, 0, true)) {
...
void CPrivateSend::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    // When tx is 0-confirmed or conflicted, pblock is nullptr and nConfirmedHeight should be set to -1
