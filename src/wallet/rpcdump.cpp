...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Dimecoin address or script");
        }
    }
    if (fRescan)
    {
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    bool fGood = true;
    {
        ...
        nTimeBegin = chainActive.Tip() ? chainActive.Tip()->GetBlockTime() : 0;
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The dimecoin address for the private key\n"
            "\nResult:\n"
...
{    
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to dimecoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    if (!tip)
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Chain has no tip");
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see dimecoind log for details) and could "
