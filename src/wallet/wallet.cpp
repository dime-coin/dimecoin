...

    {
        ...
        for (MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (CCryptoKeyStore::Unlock(_vMasterKey))
            {
                ...
                pMasterKey.second.nDeriveIterations = static_cast<unsigned int>(pMasterKey.second.nDeriveIterations * (100.0 / elapsed));
...
void CWallet::BlockUntilSyncedToCurrentChain() {
...

    {
        ...
        if (m_last_block_processed == nullptr ||
            m_last_block_processed->GetAncestor(initialChainTip->nHeight) == initialChainTip) {
            return;
        }
    }
...
bool CWallet::SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, bool fSelectWitness, const CScript &scriptFilterPubKey) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (out.nDepth < Params().GetConsensus().nStakeMinDepth)
            continue;

        if (out.tx->tx->vout[out.i].nValue < Params().GetConsensus().nStakeMinAmount)
            continue;
    ...
    int nBytes;
    {
        ...
        assert(txNew.nLockTime < LOCKTIME_THRESHOLD);
        {
            ...
            while (true)
            {
                ...
                nBytes = CalculateMaximumSignedTxSize(txNew, this) + 2;
                if (nBytes < 0) {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, CMutableTransaction& txNew, CAmount blockReward, unsigned int &nTimeTx)
{
    ...
    for (const auto& pcoin : setCoins)
    {
        ...
        if (header.GetBlockTime() + params.nStakeMinAge > nTimeTx - nMaxStakeSearchInterval)
            continue; // only count coins meeting min age requirement

        for (unsigned int n=0; n<nHashDrift && !fKernelFound; n++)
        {
            ...
            if (valid)
            {
                ...
                if (header.GetBlockTime() + nStakeSplitAge > nTimeTx)
