...
void GovernanceList::updateGobjects()
{
    ...
    for (const auto& pGovObj : objs)
    {
        ...
        // Address, Protocol, Status, Active Seconds, Last Seen, Pub Key
            ...
            std::string HexStr = pGovObj->GetDataAsHex();
...
void GovernanceList::ShowGovernanceObject(uint256 gobjectSingle) {
    ...
    std::string s = pGovObj->GetDataAsString();
    std::string dataString = pGovObj->GetDataAsString();
    std::string dataHex = pGovObj->GetDataAsHex();
