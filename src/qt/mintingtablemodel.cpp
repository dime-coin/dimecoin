...
public:
    ...
    void refreshWallet()
    {
        ...
        for(const auto& wtx : vwtx) {
            ...
            bool inMempool;
            walletModel->wallet().getWalletTxDetails(wtx.tx->GetHash(), status, orderForm, inMempool, numBlocks, adjustedTime);
    ...
    {
        {
            return TransactionDesc::toHTML(walletModel->node(), walletModel->wallet(), rec, BitcoinUnits::DIME);  
        }
        return QString("");
    }
...
void MintingTableModel::updateTransaction(const QString &hash, int status)
{
    ...
    // The wallet's transaction-changed signal is already connected in the
    // constructor, so this can run before setMintingProxyModel has supplied
    // the proxy.
