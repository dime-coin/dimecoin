...
CMutableTransaction::CMutableTransaction(const CTransaction& tx) : vin(tx.vin), vout(tx.vout), nVersion(tx.nVersion), nLockTime(tx.nLockTime), strTxComment(tx.strTxComment) {}
...
std::string CTransaction::ToString() const
{
    ...
    str += strprintf("CTransaction(hash=%s, ver=%d, vin.size=%u, vout.size=%u, nLockTime=%u, strTxComment=%s)\n",
        ...
        vout.size(),
        nLockTime,
        strTxComment.substr(0,TX_COMMENT_LIMIT).c_str());
