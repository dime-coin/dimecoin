...
// NOTE: Unlike wallet RPC (which use Dimecoin values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (ptip->nHeight >= Params().GetConsensus().nFirstPoSBlock) {
        newBlocks = true;
    }
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Dimecoin is not connected!");
