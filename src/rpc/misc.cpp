...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The dimecoin address to validate\n"
            "\nResult:\n"
            "{\n"
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The dimecoin address to use for the signature.\n"
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "dimecoin-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\ndimecoin-cli has the option -getinfo to collect and format these in the old format."
    );
}
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    if (g_connman) {
        haveConnections = g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) > 0;
    }

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("validtime", tip && tip->nTime > 1471482000));
    obj.push_back(Pair("haveconnections", haveConnections));
    if (pwalletMain) {
