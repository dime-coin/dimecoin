            ...
            case SEEN_OBJECT_EXECUTED:
                {
                    ...
                    LogPrint(BCLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- nTriggerBlock = %d, nExpirationBlock = %d\n", nTriggerBlock, nExpirationBlock);
                    if(governance.GetCachedBlockHeight() > nExpirationBlock) {
                        LogPrint(BCLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Outdated trigger found\n");
                        remove = true;
                        CGovernanceObject* pgovobj = pSuperblock->GetGovernanceObject();
                        if(pgovobj) {
                            LogPrint(BCLog::GOBJECT, "CGovernanceTriggerManager::CleanAndRemove -- Expiring outdated object: %s\n", pgovobj->GetHash().ToString());
...
//            DBG( cout << "CSuperblockManager::CreateSuperblock Payment found " << endl; );
            ...
            // TODO: PRINT NICE N.N Dimecoin OUTPUT
...
//        DBG( cout << "CSuperblock Constructor pHoObj not a trigger, returning" << endl; );
    ...
    LogPrint(BCLog::GOBJECT, "CSuperblock -- nEpochStart = %d, strAddresses = %s, strAmounts = %s, vecPayments.size() = %d\n",
...
//    DBG( cout << "CSuperblock::ParsePaymentSchedule vecParsed1.size() = " << vecParsed1.size() << endl; );
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        if (!address.IsValid()) {
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid Dimecoin Address : " <<  vecParsed1[i];
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
...
bool CSuperblock::IsValid(const CTransactionRef& txNew, int nBlockHeight, CAmount expectedReward, CAmount actualReward)
{
    ...
    LogPrint(BCLog::GOBJECT, "CSuperblock::IsValid nOutputs = %d, nPayments = %d, strData = %s\n",
