...
class HybridPoSRegtestTest(BitcoinTestFramework):
    ...
    def restart_with_staking(self, node_index, enabled):
        self.restart_node(node_index, ["-staking={}".format(int(enabled))])
    ...
    def run_test(self):
        ...
        self.set_mocktime(self.mocktime + TARGET_SPACING)
        last_pre_activation_hash = pow_node.generatetoaddress(1, first_stake_address)[0]
        sync_blocks(self.nodes, wait=0.1, timeout=30)
        assert_equal(pow_node.getblock(last_pre_activation_hash)["height"], FIRST_POS_HEIGHT - 1)

        self.log.info("Producing the first PoS block with wallet one")
        first_pos_hash = self.wait_for_pos_block(1, FIRST_POS_HEIGHT - 1)
        ...
        self.log.info("Confirming PoW remains valid after PoS activation")
        self.set_mocktime(self.mocktime + TARGET_SPACING)
        post_activation_pow_hash = pow_node.generatetoaddress(1, pow_node.getnewaddress())[0]
        sync_blocks(self.nodes, wait=0.1, timeout=30)
        post_activation_pow = observer.getblock(post_activation_pow_hash, 2)
        assert_equal(post_activation_pow["height"] >= FIRST_POS_HEIGHT, True)
        ...
        self.log.info("Restarting the observer and verifying the hybrid chain persists")
        self.restart_with_staking(3, False)
        assert_equal(observer.getbestblockhash(), best_hash)
        assert_equal(observer.getblock(first_pos_hash, 2)["confirmations"] > 0, True)
        assert_equal(observer.getblock(second_pos_hash, 2)["confirmations"] > 0, True)
        assert_equal(observer.verifychain(4, 0), True)
